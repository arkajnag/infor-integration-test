...
import io.CarPortal.Model.ErrorModel;
...
public class UserServiceTest extends BaseTest {
	...
	private final String userServiceURLGetByUserid = getProps().getProperty("USER_SERVICE_URL_GET_BY_USERID");
	...
	public void tcVerifyNewUserServiceTest(Integer userid, String username) {
		try {
			...
			Response responseObj = WebServiceUtilities.postRequest(userHostURL + userServiceURLCreateNewUser, headers,
					jsonPayloadString);
			Assert.assertTrue(CommonUtilities.verifyStatusCode.test(responseObj.getStatusCode()),
					"Expected HttpResponse of CREATED or SUCCESS or OK is not received. Please review your request. Actual Response Code received:"
	...
	public void tcVerifySearchAllUsersInformationTest() {
		try {
			...
			Assert.assertTrue(CommonUtilities.greaterThan.apply(userWrapperResponseObj.getUsers().size(), 0),
					"Please review your test data.");
		} finally {
			logger.info(Thread.currentThread().getStackTrace()[2].getMethodName() + " has end its Execution");
		}
	}

	@Test(priority = 3, description = "Negative Scenario: Verify Expected Error Message received when No Data found for specific Userid")
	public void tcVerifyErrorMessageWhenNoDataForUseridFound() {
		try {
			int randomUserid = 98673;
			ErrorModel errorResponseObj = null;
			logger.info(Thread.currentThread().getStackTrace()[2].getMethodName() + " has started its Execution");
			Response responseObj = WebServiceUtilities
					.getRequest(userHostURL + userServiceURLGetByUserid + randomUserid, headers);
			Assert.assertTrue(responseObj.getStatusCode() == 404,
					"Expected Response Code not received for Data Not Found. Error Code received:"
							+ responseObj.getStatusCode());
			try {
				errorResponseObj = mapper.readValue(responseObj.asString(), ErrorModel.class);
			} catch (JsonProcessingException e) {
				String exceptionMsg = CommonUtilities.stracktraceToString.apply(e);
				logger.error("Failed in JSON De-Serializing. Exception Details:" + exceptionMsg);
				Assert.fail("Failed in JSON De-Serializing. Exception Details:" + exceptionMsg);
			}
			Assert.assertEquals(errorResponseObj.getErrorMessage(), "No Data Present for User:"+randomUserid+". Please try with some other Userid", "Expected Error Message not received. Please review code.");
		} finally {
			logger.info(Thread.currentThread().getStackTrace()[2].getMethodName() + " has end its Execution");
		}
	}
